...
*/
int sja1000_init(__uint32_t baddr,	//!< Base address of the circuit
    ...
    )
{
	...
	if (pmode != 1)
		assert
		    ("This driver has only been implemented for pelican mode (so far...)"
		     == 0);;
...
int sja1000_read(const char *buffer, int buff_len)
{
	...
	if (_can_RxBuff[_can_rxidx_out].frinfo.FF) {
		...
		memcpy(frmtbuff->data,
		       _can_RxBuff[_can_rxidx_out].format.EFF.data, dlc);
	} else {
		frmtbuff->id = _can_RxBuff[_can_rxidx_out].format.SFF.id;
		memcpy(frmtbuff->data,
		       _can_RxBuff[_can_rxidx_out].format.SFF.data, dlc);
...
void isr_RI()
{
	...
	memcpy(&_can_RxBuff[_can_rxidx_in],
	       &_sja1000->pelican.fracc.frame, sizeof(sja1000_pelican_frame_t)
	    );
