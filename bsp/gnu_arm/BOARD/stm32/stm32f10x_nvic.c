...
void NVIC_Init(NVIC_InitTypeDef * NVIC_InitStruct)
{
	...
	assert(IS_NVIC_PREEMPTION_PRIORITY
	       (NVIC_InitStruct->NVIC_IRQChannelPreemptionPriority));
	assert(IS_NVIC_SUB_PRIORITY
	...
	if (NVIC_InitStruct->NVIC_IRQChannelCmd != DISABLE) {
		...
		tmppriority =
		    (u32) NVIC_InitStruct->NVIC_IRQChannelPreemptionPriority <<
		    tmppre;
		tmppriority |=
		...
		tmpreg =
		    NVIC->Priority[(NVIC_InitStruct->NVIC_IRQChannel >> 0x02)];
		tmpmask =
		    (u32) 0xFF <<
		    ((NVIC_InitStruct->NVIC_IRQChannel & (u8) 0x03) * 0x08);
		tmpreg &= ~tmpmask;
		...
		NVIC->Enable[(NVIC_InitStruct->NVIC_IRQChannel >> 0x05)] =
		    (u32) 0x01 << (NVIC_InitStruct->NVIC_IRQChannel & (u8)
				   0x1F);
	} else {
...
void NVIC_ClearIRQChannelPendingBit(u8 NVIC_IRQChannel)
{
	...
	NVIC->Clear[(NVIC_IRQChannel >> 0x05)] =
	    (u32) 0x01 << (NVIC_IRQChannel & (u32) 0x1F);
}
