...
29.3 .13 SMC UART Controller Programming Example
    The following initialization sequence assumes 9, 600 baud, 8 data bits,
    no parity,
    and 1 stop bit in a 25 -
    MHz system.BRG1 and SMC1 are used .1.
    Con ï ¬gure the port B pins to enable SMTXD1 and SMRXD1.Set PBPAR[24, 25]
then clear PBDIR[24, 25]
and PBODR[24, 25].
    2. Con ï ¬gure the BRG1.Write BRGC1 with 0x01 _0144.
    The DIV16 bit is not used and the divider is 162(decimal).
    The resulting BRG1 clock is 16 Ã —the preferred bit rate.3. Connect BRG1 to
    SMC1 using the SI.Clear SIMODE[SMC1,
				   SMC1CS].4. Assuming one RxBD at the
    beginning of dual - port RAM followed by one TxBD,
    write RBASE with 0x0000 and TBASE with 0x0008.5. Write 0x0091 to CPCR to
    execute the INIT RX AND TX PARAMETERS command.6. Initialize the SDMA con ï
    ¬guration register (SDCR) to 0x0001.7. Write RFCR and TFCR with 0x10 for
    normal operation .8.Write MRBLR with the maximum number of bytes per receive
    buffer.Assume 16 bytes, so MRBLR = 0x0010
	.9.Write MAX_IDL with 0x0000 in the SMC UART -
	    speci ï ¬c parameter RAM to disable the MAX_IDL functionality for
this
		example .10.Clear BRKLN and BRKEC in the SMC UART -
		    speci ï ¬c parameter RAM .11.Set BRKCR to 0x0001;
if a
STOP TRANSMIT COMMAND is issued, one break
	    character is sent .12.
	    Initialize the RxBD.Assume the Rx buffer is at 0x0000 _1000
	    in main memory.Write 0xB000 to Rx_BD_Status,
	    0x0000 to Rx_BD_Length(not required),
	    and 0x0000 _1000 to Rx_BD_Pointer .13.
	    Assuming the Tx buffer is at 0x00002000 in main memory and
	    contains ï ¬ve 8 - bit characters,
	    write 0xB000 to Tx_BD_Status, 0x0005 to Tx_BD_Length,
	    and 0x00002000 to Tx_BD_Pointer .14.Write 0xFF to the SMCE register
to clear any previous events.
    15. Write 0x17 to the SMCM register to enable all possible SMC interrupts.
    16. Write 0x00000010 to the CIMR so the SMC1 can generate a system
    interrupt.Initialize the CICR.
    17. Write 0x4820 to SMCMR to con ï ¬gure normal operation(not loopback),
    ...
    additional write ensures that the TEN and REN bits are enabled last. After 5
    bytes are sent,
    the TxBD is closed.
    The receive buffer closes after receiving 16 bytes. Subsequent data causes a
busy(out - of - buffers)
condition since only one RxBD is ready.
#endif
int bsleep(int time)
{
	int i, j, k, t = 0x123;
...
// 2)
//Conï¬gure the BRG1. Write BRGC1 with 0x01_0144. The DIV16 bit is not used
//and the divider is 162 (decimal). The resulting BRG1 clock is 16Ã— the preferred bit
//rate.

	{
		...
		__uint32_t brgclk =
		    (oscclk * (plprcr_p->f.MF + 1)) / (1 << sccr_p->f.DFBRG);
...
// 6. Initialize the SDMA conï¬guration register (SDCR) to 0x0001.
	SDCR = 0x0001;
...
// Maximum receive buffer length. Deï¬nes the maximum number of bytes the CP writes
// to a receive buffer before it goes to the next buffer.

	//smc_param->MRBLR = 0x0010;
	smc_param->MRBLR = 0x0001;

//9. Write MAX_IDL with 0x0000 in the SMC UART-speciï¬c parameter RAM to
...
//10. Clear BRKLN and BRKEC in the SMC UART-speciï¬c parameter RAM.
...
//13. Assuming the Tx buffer is at 0x00002000 in main memory and contains ï¬ve 8-bit
...
/*
NOTE
34.5.1 CPM Interrupt Conï¬guration Register (CICR)
The CPM interrupt conï¬guration register (CICR) deï¬nes CPM interrupt request levels, the
priority between the SCCs, and the highest priority interrupt.

ehh??
*/

//17. Write 0x4820 to SMCMR to conï¬gure normal operation (not loopback), 8-bit
