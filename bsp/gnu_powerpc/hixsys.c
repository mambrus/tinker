...
extern "C" {
	...
	int _execve(char *name, char **argv, char **env) {
		MON(hixs.execve);
		return hixs.execve(name, argv, env);
	}
	...
	int _gettimeofday(struct timeval *tp, struct timezone *tzp) {
		MON(hixs.gettimeofday);
		return hixs.gettimeofday(tp, tzp);
	}
	int _isatty(int file) {
		MON(hixs.isatty);
		return hixs.isatty(file);
	}
	int _kill(int pid, int sig) {
		MON(hixs.kill);
		return hixs.kill(pid, sig);
	}
	int _link(char *old, char *new) {
		MON(hixs.link);
		return hixs.link(old, new);
	}
	int _lseek(int file, int ptr, int dir) {
		MON(hixs.lseek);
		return hixs.lseek(file, ptr, dir);
	}
	...
	int _read(int file, char *ptr, int len) {
		MON(hixs.read);
		return hixs.read(file, ptr, len);
	}
	caddr_t _sbrk(int incr) {
		MON(hixs.sbrk);
		return hixs.sbrk(incr);
	}
	int _settimeofday(const struct timeval *tp, const struct timezone *tzp) {
		MON(hixs.settimeofday);
		return hixs.settimeofday(tp, tzp);
	}
	int _stat(const char *file, struct stat *st) {
		MON(hixs.stat);
		return hixs.stat(file, st);
	}
	clock_t _times(struct tms * buf) {
		MON(hixs.times);
		return hixs.times(buf);
	}
	int _unlink(char *name) {
		MON(hixs.unlink);
		return hixs.unlink(name);
	}
	int _wait(int *status) {
		MON(hixs.wait);
		return hixs.wait(status);
	}
	int _write(int file, char *ptr, int len) {
		MON(hixs.write);
		return hixs.write(file, ptr, len);
	}

#if !defined( _NO_HIXS_SYSCALLMON_ )
	void _syscall_mon(void *sys_func_ptr) {
		hixs.syscall_mon(sys_func_ptr);
	}
#endif
