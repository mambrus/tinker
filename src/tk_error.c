...
void tk_trap(int ec){
	...
	#define MAX_LEN 80
		...
		if (ec == 0){
			...
			for (i=0,m=1;i<15;i++,m=m<<1){
				if (ec & m){
					...
					console_write(astr,strlen(astr));
					console_write(": ",2);
					console_write( trapcode_srings[i+1], strlen(trapcode_srings[i+1]));
					console_write("\n\r",2);
				}
			}
		}
		...
		if (ec == 0){
			...
			TK_USE_EMRGCY_CONSOLE( trapcode_srings[16], strlen(trapcode_srings[16]));
			_ntos(astr,ec,MAX_LEN);
			...
			for (i=0,m=1;i<15;i++,m=m<<1){
				if (ec & m){
					...
					TK_USE_EMRGCY_CONSOLE( trapcode_srings[i+1], strlen(trapcode_srings[i+1]));
					TK_USE_EMRGCY_CONSOLE("\n\r",2);
				}
			}
		}
